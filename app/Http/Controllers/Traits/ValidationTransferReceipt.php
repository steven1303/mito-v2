...
trait ValidationTransferReceipt {

    // Status Transfer Receipt (String) [ Draft, Request, Approved ]

    function buttonAction($data, $access ){
        $action = "";
        $title = "'".$data->adj_no."'";
        $transfer_receipt_detail = "javascript:ajaxLoad('".route('transfer.receipt.form', ['id' => $data->id])."')";
        if($access['edit']){
            $action .= '<a href="'.$transfer_receipt_detail.'" class="btn btn-primary btn-xs"> Open</a> ';
        }
        ...
        if($access['print'] && ($data->status == 'Request' || $data->status == 'Approved')){
            $action .= '<button id="'. $data->id .'" onclick="print_transfer_receipt('. $data->id .')" class="btn btn-default btn-xs"> Print</button> ';
        }
        return $action;
    }
    ...
    function accessTransferReceipt($auth, $permission){
